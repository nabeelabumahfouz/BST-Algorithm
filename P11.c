...
void searchBST(Node *rt, char *keyWord, int left, int right);
...
int main(int argc, char const *argv[])
{
    ...
    // SEARCH TREE:
    searchBST(rt, keyword, 1, uniqueWordCount);
...
int FileRead(char *wordArr[2045])
{
    ...
    printf("Please enter the name of the file (for P11): ");
    scanf("%s", fileName);
...
Node *insertNode(Node *rt, Node *temp, int left, int right)
{
    ...
    // Get the root index from root table for current range:
    int val = rootT[left][right];
    int ind = val - 1;
    ...
    // Recursively build left & right subtree (left = left -> val-1 , right = val+1 -> right):
    rt->left = insertNode(rt->left, temp, left, val - 1);
    rt->right = insertNode(rt->right, temp, val + 1, right);

    return rt;
}


//Function used to search the BST tree for KEY recursively:
void searchBST(Node *rt, char *keyWord, int left, int right) 
{
    ...
    // Printing out whihc way tree is going and what it has compared with (as specified in guide handout):
    x = strcmp(keyWord, rt->word);

    if (x < 0)
    {
        printf("Compared with %s (%0.3lf), go to LEFT subtree.\n", rt->word, mainT[left][right]);
        searchBST(rt->left, keyWord, left, rootT[left][right] - 1);
    }
    else if (x > 0)
    {
        printf("Compared with %s (%0.3lf), go to RIGHT subtree.\n", rt->word, mainT[left][right]);
        searchBST(rt->right, keyWord,  rootT[left][right] + 1, right);
    }
    else
    {
        printf("Compared with %s (%0.3lf), ", rt->word, mainT[left][right]);
        printf("FOUND KEY!!!\n\n");
        return;
    }
}
